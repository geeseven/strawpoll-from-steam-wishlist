...
html_title = "<title>poll maker</title>"
...
def get_wishlist(url):
    ...
    try:
        ...
        return("ERROR! Could not connect to Steam, some sort of networking issue. Raw error message:<br><br>{}".format(e))
    else:
        # Check to make sure we got valid json.  This will also catch non 200 replies.
        try:
            wishlist_json = req.json()
        except json.decoder.JSONDecodeError as e:
            return("ERROR! Steam did not provide valid data. Raw json error message:<br><br>{}".format(e))
...
def make_poll(games):
    ...
    poll_info = {"title": "This is a test poll.", "options": games, "multi": "false"}
    try:
        ...
        return("ERROR! Could not connect to Srawpoll, some sort of networking issue. Raw error message:<br><br>{}".format(e))
    else:
        # Check to make sure we got valid json.  This will also catch non 200 replies.
        try:
            poll_reply = poll.json()
        except json.decoder.JSONDecodeError as e:
            return("ERROR! Connected to Strawpoll, but could not create poll. Raw json error:<br><br>{}".format(e))
    return(poll_reply)
    ...
    if (type(wish_list)) is not str:
        ...
        if type(game_list) is list:
            ...
            if type(poll) is not str:
                # print created poll
                return(html_title + "https://www.strawpoll.me/"+str(poll["id"]))
            else:
                # print error creating poll
                return(html_title + poll)
        # print game list error
        else:
            return(html_title + game_list)
    # print Steam error
    else:
        return(html_title + wish_list)
